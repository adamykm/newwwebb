...
auth.post('/register', async (c) => {
  ...
  return c.json({
    user: {
      ...
      isDisabled: false, isTerminated: false, globalTimeoutUntil: null,
    },
...
auth.post('/login', async (c) => {
  ...
  const cleanEmail = email.trim().toLowerCase();
  const row = await c.env.DB.prepare('SELECT * FROM users WHERE email = ?').bind(cleanEmail).first();
  ...
  if (row.is_terminated) return c.json({ error: 'This account has been permanently suspended.' }, 403);
  if (row.is_disabled) return c.json({ error: 'This account has been disabled. Contact an administrator.' }, 403);

  let userRole = row.role as 'user' | 'admin';

  // Promote existing account to admin if ADMIN_EMAIL+ADMIN_PASSWORD match
  const adminEmail = c.env.ADMIN_EMAIL?.trim().toLowerCase();
  const adminPassword = c.env.ADMIN_PASSWORD;
  if (adminEmail && adminPassword && cleanEmail === adminEmail && password === adminPassword) {
    if (userRole !== 'admin') {
      await c.env.DB.prepare('UPDATE users SET role = ? WHERE id = ?').bind('admin', row.id).run();
    }
    userRole = 'admin';
  }
  ...
  user.role = userRole;
  const token = await createSessionToken({ userId: user.id, role: userRole }, c.env);
  const secure = c.req.url.startsWith('https');
  ...
  return c.json({
    user: {
      ...
      role: userRole, avatarColor: user.avatarColor, avatarUrl: user.avatarUrl,
      ...
      isDisabled: user.isDisabled ?? false, isTerminated: user.isTerminated ?? false,
      globalTimeoutUntil: user.globalTimeoutUntil ?? null,
    },
...
auth.get('/me', async (c) => {
  ...
  if (row.is_terminated || row.is_disabled) return c.json({ user: null });

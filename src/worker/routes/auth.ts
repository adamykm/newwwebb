...
auth.post('/register', async (c) => {
  ...
  if (!/^[a-zA-Z0-9_]{3,20}$/.test(cleanUser)) {
    return c.json({ error: 'Username must be 3-20 alphanumeric chars' }, 400);
  }

  const existing = await c.env.DB.prepare(
    'SELECT id FROM users WHERE email = ? OR username = ?'
  ).bind(cleanEmail, cleanUser).first();
  ...
  return c.json({ user: { id: userId, email: cleanEmail, username: cleanUser, role, avatarColor } }, 201, {
...
auth.post('/login', async (c) => {
  ...
  const secure = c.req.url.startsWith('https');
  return c.json({ user: { id: user.id, email: user.email, username: user.username, role: user.role, avatarColor: user.avatarColor } }, 200, {
    'Set-Cookie': sessionCookie(token, secure),
  });
});
...
auth.get('/me', async (c) => {
  ...
  const user = rowToUser(row as Record<string, unknown>);
  return c.json({ user: { id: user.id, email: user.email, username: user.username, role: user.role, avatarColor: user.avatarColor } });
});
